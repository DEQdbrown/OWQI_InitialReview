...
#See 'w.all.avg.csv' for formatting of input file; created by R.Leferink 2/1/11
#No output appears in R; writes to .pdf files in R directory folder

owqi.plot.needy<-function(data,x,y, endWY){
        site1<-as.vector(data$combo_name[1])                 
        station1<-as.vector(data$Station[1])             
        png(filename=paste('//deqlab1/wqm/owqi/30yr graphs/',"OWQI_",site1, " #", station1, ", ", endWY-30, "-", endWY, ".png", sep=""), width=11, height=8.5, units = "in", res = 100)
        par(mar=c(4,4,4,4), oma=c(.4, 0,1.7, 0))   #mfrow=c(2,2), 
...
# #bod5
   plot(y~x, type="l", ylim=c(10, 100), xlim=c(endWY-30, endWY), col="#CED1D8", lwd=5, 
     ...
     lines(data$bod_si~x, lwd=2, col="#4568B5", lty=6);  axis(4, col.axis="#767592")
         ...
         legend("bottomright", c("OWQI", "BOD Subindex"), col=c("#CED1D8","#4568B5"),
       lty=c(1, 6), lwd=c(5, 2), bty="n", cex=.8)
...
#  #ntotal.n/ n_si  
   plot(y~x, type="l", ylim=c(10, 100), xlim=c(endWY-30, endWY), col="#CED1D8", lwd=5, 
     ...
     legend("bottomright", c("OWQI", "N Subindex"), col=c("#CED1D8","#4568B5"),
        lty=c(1, 6), lwd=c(5, 2), bty="n", cex=.8)
...
# #p
   plot(y~x, type="l", ylim=c(10, 100), xlim=c(endWY-30, endWY), col="#CED1D8", lwd=5, 
     ...
     legend("bottomright", c("OWQI", "P Subindex"), col=c("#CED1D8","#4568B5"),
        lty=c(1, 6), lwd=c(5, 2), bty="n", cex=.8)  
